...
def show_equities_page():
    ...
    if uploaded_price_file is not None:
        ...
        df_price = pd.read_csv(r"Input_data\clean_prices.csv")
    ...
    if uploaded_port_file is not None:
        ...
        df_port = pd.read_csv(r"Input_data\MPFPortfolio.csv")

    # Prepare price dataset
    df = df_price.copy()
    date_col = df['Date'] if 'Date' in df.columns else None
    price_df = df.drop(columns=['Date']) if date_col is not None else df.copy()
    ...
    # Dataset selection
    dataset_choice = st.sidebar.radio("Select Dataset to Analyze:", ["Price Data", "Portfolio Returns and Weights", "Visualization"])
    ...
    if dataset_choice == "Portfolio Returns and Weights":
        ...
        if required_cols.issubset(df_port.columns):
            ...
            if curr_wts.isnull().any() or targ_wts.isnull().any():
                st.error("Portfolio weights contain missing values after reindexing. Check stock names alignment with price CSV.")
            else:
                ...
                st.dataframe(styled_summary, width=None, height=None)
        else:
        ...
        view_option = st.radio("View:", ["Prices", "Daily Returns", "Excess Returns", "Covariance Matrix", "Correlation Matrix", "Statistics"])
        ...
        if view_option == "Prices":
            ...
            corr_df = pd.DataFrame(cov_df.values / outer_std, index=returns_only.columns, columns=returns_only.columns)
        ...
        if 'Stock' in df_port.columns:
            ...
            value_metric = st.radio(
                "Select value metric to plot:",
                [m for m in ['Est_Acquisition_Cost', 'Carrying_Cost', 'Market_Value'] if m in df_port.columns]
            )
            ...
            return_metric = st.radio(
                "Select return metric to plot:",
                [m for m in ['Acquisition_Return', 'Return', 'Target Return'] if m in df_port.columns]
            )
